...
public SayFlop15            = 0 // displays last 15 players
...
new g_HudSync_SpecInfo  // KWo - 02.10.2006
...
{
	// Register plugin.
	register_plugin("StatsX2", AMXX_VERSION_STR, "AMXX Dev Team")
	register_dictionary("statsx2.txt")
	...
	register_clcmd("say /flop15", "cmdFlop15", 0, "- display worst 15 players (MOTD)") // KWo - thanks KRoT@L
	...
	register_clcmd("say_team /flop15", "cmdFlop15", 0, "- display worst 15 players (MOTD)") // KWo - thanks KRoT@L	
	...
	g_HudSync_SpecInfo = CreateHudSyncObj() // KWo - 02.10.2006
}
...
public plugin_cfg()
{
	...
	server_cmd(addStast, "ST_SAY_FLOP15", "SayFlop15") // KWo thanks KRoT@L - 02.10.2006
...
{
	set_hudmessage(255, 255, 255, 0.02, 0.96, 2, 0.05, 0.1, 0.01, 3.0, -1) // KWo - 02.10.2006
}
...
// Get and format flop 15.
format_flop15(sBuffer[MAX_BUFFER_LENGTH + 1]) // KWo - thanks KRoT@L
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8]
	new iLen = 0
	...
	for (new i = iMax-1; i >= iMin && MAX_BUFFER_LENGTH - iLen > 0; i--)
	{
		get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
		replace_all(t_sName, MAX_NAME_LENGTH, "<", "[")
		replace_all(t_sName, MAX_NAME_LENGTH, ">", "]")
		iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%2d %-22.22s %6d %6d %6d %6d %4d %3.0f%% %3.0f%%^n", iMax-i, t_sName, izStats[STATS_KILLS], 
						izStats[STATS_DEATHS], izStats[STATS_HITS], izStats[STATS_SHOTS], izStats[STATS_HS], effec(izStats), accuracy(izStats))
	}
}
...
// Display MOTD flop15 ranked.
public cmdFlop15(id)  // KWo - thanks KRoT@L
{
	if (!SayFlop15)
	{
		client_print(id, print_chat, "%L", id, "DISABLED_MSG")
	...
	format_flop15(g_sBuffer)
	show_motd(id, g_sBuffer, "Flop 15")
...
public eventStartRound()
{
	...
	if (iRoundTime >= floatround(get_cvar_float("mp_roundtime") * 60,floatround_floor))
	{
#if defined STATSX_DEBUG
...
public eventShowRank(id)
{
	if (SpecRankInfo && g_izSpecMode[id])
	{
		...
		if (is_user_connected(iPlayer))
		{
			...
			set_hudtype_specmode()
			ShowSyncHudMsg(id, g_HudSync_SpecInfo, "%L", id, "X_RANK_IS", t_sName, iRankPos, iRankMax) // KWo - 02.10.2006
//			show_hudmessage(id, "%L", id, "X_RANK_IS", t_sName, iRankPos, iRankMax) // KWo - 02.10.2006
		}
	}
