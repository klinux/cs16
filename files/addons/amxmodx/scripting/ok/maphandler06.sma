...
#define UNWANTEDMAPSCFGFILE	"unwantedmaps.dat"
#define WANTEDMAPSCFGFILE	"standardmaps.dat"
...
stock charOccurances(string[],matchchar[]) {
	...
	new occurances = 0
	//server_print("[AMXX] Occurances inited to %d", occurances)
...
AddWantedMaps(maps[MAXMAPS][MAPLENGTH], &mapsCount) {
	//server_print("[AMXX] [%s] Adding extra maps defined in cfg file...", PLUGINNAME)
	// Filename must exist
	...
	while (mapsCount < MAXMAPS && (pos = read_file(g_wantedMapsFile, pos, mapline, MAPLENGTH - 1, len))) {
		...
		if (already) {
			//server_print("[AMXX] [%s] Already in list because we found it earlier in maps directory: %s (len %d)", PLUGINNAME, mapline, len)
			continue
		}
		else if ( !InvalidMap(mapline) ) {
			//server_print("[AMXX] [%s] Adding extra map not in maps dir: %s. Map is %s. (len %d)", PLUGINNAME, mapline, is_map_valid(mapline) ? "VALID" : "INVALID", len)
...
writemaps() {
	...
	do {
		//debugcounter++
		//server_print("[AMXX] [%s] DEBUG - Sending pos: %d", PLUGINNAME, pos)
		pos = read_dir("maps/", pos, fileName, MAPLENGTH + 3, len)
		//server_print("[AMXX] [%s] DEBUG - Reading %s at pos %d with length %d (counter: %d)", PLUGINNAME, fileName, pos, len, debugcounter)
		if (len >= 5) {
			bspPos = len - 4
			if (pos && containi(fileName,".bsp") == bspPos) {
				copy(mapName,bspPos,fileName)
				//server_print("[AMXX] %s: DEBUG - found map: %s", PLUGINNAME, mapName)
				if (is_map_valid(mapName)) {
					...
					if (checkInvalid && InvalidMap(mapName)) {
						//server_print("[AMXX] [%s] Not adding %s because we don't want it...", PLUGINNAME, mapName)
						continue
					}
					//else
						//server_print("[AMXX] [%s] Adding %s...", PLUGINNAME, mapName)
					maps[mapsCount] = mapName
					mapsCount++
				}
				//else
					//server_print("[AMXX] [%s] DEBUG - %s is an invalid map", PLUGINNAME, mapName)
			}
			//else
				//server_print("[AMXX] [%s] DEBUG - File ^"%s^" does not end with ^".bsp^" so it's probably not a map file.", PLUGINNAME, fileName)
		}
		//else
			//server_print("[AMXX] [%s] DEBUG - File ^"%s^" is probably not a map file since its name is too short.", PLUGINNAME, fileName)
	}
	...
	//server_print("[AMXX] [%s] DEBUG - Done looking through maps/ directory. Counter: %d", PLUGINNAME, debugcounter)
	...
	server_print("[AMXX] [%s] %sFound %d valid maps in server.", PLUGINNAME, mapsCount == 0 ? "WARNING - " : "", mapsCount)
	...
	if (file_exists(outfile))
		if (!delete_file(outfile)) {
			server_print("[AMXX] [%s] ERROR, Couldn't delete %s! Failed initializing %s.", PLUGINNAME,outfile,PLUGINNAME)
			return 0
		}

	log_amx("About to write to %s.", outfile)
	if (!write_file(outfile,mapsCountString)) {
		server_print("[AMXX] [%s] ERROR, Couldn't write to %s! Failed initializing %s.", PLUGINNAME,outfile,PLUGINNAME)
		return 0
	}
...
public listmaps_fn(id, const COMMANDUSED[]) {
	...
	if (mcs_len > 0)
		...
		console_print(id, "[AMXX] [%s] Odd error. You aren't fiddling with the wrong config files, are you?", PLUGINNAME)
		return PLUGIN_HANDLED
	}
	...
	if (argCount > 1) {
		...
		for (new i = 0; i < strlen(arg); i++) {
			if (!isdigit(arg[i])) {
				//console_print(id, "[AMXX] [%s] You must specify a numerical value.", PLUGINNAME)
...
public mapcycle(id,level,cid) {
	if (!cmd_access(id,level,cid,2)) {
		console_print(id,"[AMXX] [%s] Incorrect use. Specify a file that should be written as mapcycle. Mapcycle.txt replaces standard mapcycle.", PLUGINNAME)
		return PLUGIN_HANDLED
	}
	...
	if (!inited || !file_exists(infile)) {
		console_print(id,"[AMXX] Map list initialization failed.")
		return PLUGIN_HANDLED
	}
	...
	if (file_exists(outfile)) {
		if (!delete_file(outfile)) {
			console_print(id,"[AMXX] [%s] Failed to delete already existing '%s'.", PLUGINNAME,outfile)
			return PLUGIN_HANDLED
		}
	}
	...
	for (new i = 0;i < mapsCount && i < MAXMAPSINMAPCYCLE;i++) {
		...
		if (!write_file(outfile,maps[i])) {
			console_print(id,"[AMXX] [%s] Failed while trying to write '%s' to '%s'.", PLUGINNAME,textline,outfile)
			return PLUGIN_HANDLED
		}
		else {
			mapsCount2++
	...
	console_print(id,"[AMXX] [%s] Successfully wrote %d maps to %s.", PLUGINNAME,mapsCount2,outfile)

	return PLUGIN_HANDLED
}

writemapsini() {
	server_print("[AMXX] [%s] Starting dump of maps...", PLUGINNAME)

	// Check if we inited properly.
	new infile[128]
	get_cvar_string(CVAR_MHINIFILE,infile,127)
	if (!inited || !file_exists(infile)) {
		server_print("[AMXX] [%s] Map list initialization failed.", PLUGINNAME)
		return PLUGIN_HANDLED
	}
	...
	if (mcs_len > 0)
		...
		server_print("[AMXX] [%s] Odd error. You aren't fiddling with the wrong config files, are you?", PLUGINNAME)
		return PLUGIN_HANDLED
	}
	...
	if (!file_exists(mapsini)) {
		...
		if (!initMapsini(mapsini)) {
			server_print("[AMXX] [%s] Error: couldn't initialize '%s'!", PLUGINNAME, mapsini)
			return PLUGIN_HANDLED
		}
		...
		for (new i = 0;i < mapsCount && i < MAXMAPS;i++) {
			if (!write_file(mapsini,maps[i])) {
				server_print("[AMXX] [%s] Failed while trying to write '%s' to '%s'.", PLUGINNAME, maps[i], mapsini)
				return PLUGIN_HANDLED
			}
			else {
				mapsCount2++
		...
		server_print("[AMXX] [%s] Successfully wrote %d maps to '%s'.", PLUGINNAME,mapsCount2,mapsini)
		return PLUGIN_HANDLED
	}
	...
	if (file_exists(tempfile)) {
		if (!delete_file(tempfile)) {
			server_print("[AMXX] [%s] Error: couldn't delete temporary initialization file '%s'!", PLUGINNAME,tempfile)
			return PLUGIN_HANDLED
		}
	}

	if (!initMapsini(tempfile)) {
		server_print("[AMXX] [%s] Error: couldn't initialize '%s'!", PLUGINNAME,tempfile)
		if (file_exists(tempfile)) {
			if (!delete_file(tempfile))
				server_print("[AMXX] [%s] Error: couldn't delete temporary initialization file '%s'!", PLUGINNAME,tempfile)
		}
...
*/
			...
			if (textline[0] == ';' && occurances  == 2) {
				...
				if (!matches) {
					if (!write_file(tempfile,textline)) {
						server_print("[AMXX] [%s] Error: couldn't write to temporary file '%s'!", PLUGINNAME,tempfile)
						break
					}
				}
			}
	...
	if (!delete_file(mapsini)) {
		server_print("[AMXX] [%s] Error: couldn't delete old '%s' to replace it with a new one! New one is saved to '%s'.", PLUGINNAME,mapsini,tempfile)
		return PLUGIN_HANDLED
	}
	...
	do {
		...
		if (line) {
			if (!write_file(mapsini,textline)) {
				server_print("[AMXX] [%s] Error: couldn't write to map file '%s'! Map list saved to '%s'.", PLUGINNAME,mapsini,tempfile)
				return PLUGIN_HANDLED
			}
			else
				writtenLines++
	...
	server_print("[AMXX] [%s] Successfully wrote %d lines to '%s'.", PLUGINNAME,writtenLines,mapsini)
...
initMapsini(mapsini[]) {
	...
	new amxversion[32]
	new date_time[64]	
	get_time("%m/%d/%y %H:%M:%S", date_time, 63) 
	if (!write_file(mapsini,"; Maps configuration file"))
		return 0
	format(formattedLine,127,"; Automatically generated %s by %s version %s", date_time, PLUGINNAME, VERSION)
...
public plugin_init() {
	...
	*/
	...
	server_print("[AMXX] [%s] Initializing...", PLUGINNAME)
	server_print("[AMXX] Starting map: %s", currentMap)
	log_message("[AMXX] Starting map: %s", currentMap)       
			
	if (writemaps()) {
		inited = true
		writemapsini()
		server_print("[AMXX] [%s] ...done!", PLUGINNAME)
	}
	else
		server_print("[AMXX] [%s] ERROR - Failed initializing!", PLUGINNAME)
}