...
#define MAX_MENU_DATA 72 // increase this number if you have this error:
...
public plugin_precache()
{
	...
	register_dictionary("statscfg2.txt")
...
displayCfgMenu(id, pos)
{
	...
	{
		keys |= (1<<k)
		len += format(menu_body[len], 511-len, g_coloredMenus ? "%d. %L\y\R%L^n\w" : "%d. %L %L^n", ++k, id, g_menuData[a], id, get_xvar_num(g_menuDataId[a]) ? "ON" : "OFF") // KWo
	}
