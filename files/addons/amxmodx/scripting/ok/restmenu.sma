...
// bot weapons restrictions start
	if (action == 2)
	{
		if (a < 24)
		{
	...
	else
	{
		...
		if (a < 24)
		{
			g_szWeapRestr[a + 1] = sz[0]  				// primary and secondary weapons
		}
		else if ((a >= 24) && (a < 31))
		{
			g_szEquipAmmoRestr[a - 24] = sz[0]  	// equipments
		}
		else if (a == 31)
		{
			g_szWeapRestr[25] = sz[0]  						// shield
		}
		else if ((a > 31) && (a < 34))
		{
			g_szEquipAmmoRestr[a - 25] = sz[0]   	// primary and secondary ammo
		}
	}
	set_cvar_string("amx_restrweapons", g_szWeapRestr)
	set_cvar_string("amx_restrequipammo", g_szEquipAmmoRestr)

// bot weapons restrictions end
